...
/**
 * Type to capture the indentation as an immutable type containing a pre-filled
 * buffer to quickly be written.
 *
...
public final class Indentation implements CharSequence, Serializable {
    ...
    static {
        for (int lvl = 0; lvl < TWO_SPACES.length; lvl++)
            TWO_SPACES[lvl] = new Indentation(2, ' ', lvl);
        for (int lvl = 0; lvl < FOUR_SPACES.length; lvl++)
            FOUR_SPACES[lvl] = new Indentation(4, ' ', lvl);
        for (int lvl = 0; lvl < TABS.length; lvl++)
            TABS[lvl] = new Indentation(1, '\t', lvl);
    }
    ...
    /**
     * A reusable constant for no indentation at all (even after calls to
     * {@link #increase()}).
     */
     ...
     *
     * @param level
     *                  The number of tabs for this indentation.
     ...
     * <code>width * level</code> spaces. This indentation increases or decreases by
     * <code>width</code> spaces at a time.
     *
     * @param width
     *                  The number of spaces for a single indentation level (often 2
     *                  or 4).
     * @param level
     *                  The current indentation level (multiply this with the width
     *                  for the initial number of spaces).
     * @return The indentation level as <code>level</code> multiples of
     *         <code>width</code> spaces.
     */
    public static Indentation spaces(int width, int level) {
        return width < 0 ? spaces(DEFAULT.ch == ' ' ? DEFAULT.width : 4, level)
                : width == 0 ? NONE : width == 2 && level < TWO_SPACES.length ? TWO_SPACES[max(level, 0)] : width == 4 && level < FOUR_SPACES.length ? FOUR_SPACES[max(level, 0)] : new Indentation(width, ' ', level);
    }

    /**
     * Internal 'factory' method that tries to resolve a constant indentation
     * instance before returning a new object.
     *
     * @param width
     *                  The indentation width for one indentation unit
     * @param ch
     *                  The character used in the indentation
     * @param level
     *                  The numer of logical indentations to apply
     * @return the requested indentation either as a resolved constant instance or a
     *         new object
     */
    private static Indentation resolve(final int width, final char ch, final int level) {
        return width == 0 ? NONE : ch == ' ' ? spaces(width, level) : ch == '\t' && width == 1 ? tabs(level) : new Indentation(width, ch, level);
    }
    ...
    /**
     * @return An indentation instance with the level decreased by one (if there was
     *         indentation left to decrease).
     */
    ...
    /**
     * Makes sure that after deserialization, the constant instances are resolved
     * where possible.
     *
     * @return The deserialized object from the cache if possible or a new instance
     *         otherwise.
     */
     ...
     *
     * @param other
     *                  The other indentation object to compare with.
     * @return <code>true</code> if the other object is the same indentation object.
     */
    public boolean equals(Object other) {
        return this == other || (other instanceof Indentation && width == ((Indentation) other).width && ch == ((Indentation) other).ch && level == ((Indentation) other).level);
    }
